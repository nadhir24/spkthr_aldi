...
class Rangking extends MY_Controller
{
    ...
    public function index()
    {
         ...
         */
        $table1 = $this->initialTableSAW($pelanggan);
    ...
    private function initialTableSAW($pelanggan)
    {
        $nilai = $this->MNilai->getNilaiPelanggan();

        $dataInput = array();
        $no = 0;
        foreach ($pelanggan as $item => $itemPelanggan) {
            foreach ($nilai as $index => $itemNilai) {
                if ($itemPelanggan->kdPelanggan == $itemNilai->kdPelanggan) {
                    $dataInput[$no]['pelanggan'] = $itemPelanggan->pelanggan;
